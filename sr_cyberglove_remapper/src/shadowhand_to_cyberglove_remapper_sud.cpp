...
//own .h
#include "shadowhand_to_cyberglove_remapper_sud.h"
...
namespace shadowhand_to_cyberglove_remapper
{
 ...
 const int ShadowhandToCybergloveRemapperSud::number_hand_joints_ = 20;

ShadowhandToCybergloveRemapperSud::ShadowhandToCybergloveRemapperSud() :
  n_tilde_("~")
{
    proj_matrix_.setIdentity(number_hand_joints_,number_hand_joints_);

    joints_names_.resize(number_hand_joints_);
    ShadowhandToCybergloveRemapperSud::initNames();
    ...
    cyberglove_jointstates_sub_ = node_.subscribe(full_topic, 10, &ShadowhandToCybergloveRemapperSud::jointStatesCallback, this);

    n_tilde_.searchParam("remapper_prefix", searched_param);
    n_tilde_.param(searched_param, prefix, std::string());
    full_topic = prefix + "project_eigenspace";
    project_eigenspace_service_ = node_.advertiseService(full_topic,&ShadowhandToCybergloveRemapperSud::formProjMatrix,this);
...
void ShadowhandToCybergloveRemapperSud::initNames()
{
...
double ShadowhandToCybergloveRemapperSud::linRegression(int ordinal, double sv1, double sv2)
{
...
double ShadowhandToCybergloveRemapperSud::checkJointLimits(double angle,double lower,double upper)
{
  double new_angle;
...
double ShadowhandToCybergloveRemapperSud::linWristMapping(std::string const & joint,double const sv)
{
  double angle=0;
  if(joint=="G_WristPitch")
    ...
    ROS_ERROR("Invalid wrist joint specified in ShadowhandToCybergloveRemapperSud::linWristMapping(std::string const & joint,double const sv)");

  return angle;
}

void ShadowhandToCybergloveRemapperSud::projectOnEspace(Eigen::VectorXd & hand_joints)
{
  ...
  Eigen::VectorXd ShadowhandToCybergloveRemapperSud::getRemappedVector(std::vector<double> const & glove_values)
{
...
void ShadowhandToCybergloveRemapperSud::jointStatesCallback( const sensor_msgs::JointStateConstPtr& msg )
{
  ...
  bool ShadowhandToCybergloveRemapperSud::formProjMatrix(sr_cyberglove_remapper::project_eigenspace::Request  &req, sr_cyberglove_remapper::project_eigenspace::Response &res)
 {
  data_mutex_.lock();

...
void tick_wtracker_old() {
  static byte receive_buf[40];
  int intA, sensor;
  int ret,i;

  /* We're looping and reading until there are no more data\n"); */
  for(;;) {
    /*recv_usb(WG_fs_usb, 32, receive_buf);*/
    for(i=0;i<32;i++) receive_buf[i]=0;

    ret=usb_bulk_read(WG_fs_usb, endpoint_in, (char*)&receive_buf[0], 40, 10); /* wait up to 1ms for the data */
    if(ret < 32) 
      /* Too little read data, we're finished for this tick */
      return; 

    int cnt=0;
    while(receive_buf[0] != 'D' && cnt++ < 10) {
      for(i=0;i<39;i++) receive_buf[i] = receive_buf[i+1];
    }
    printf("Had to skip %d bytes in input data..\n",cnt);

#ifdef FOO
    printf("got %02x\n",receive_buf[0]);
    if(receive_buf[0] != 'D') {
      printf("I dont understand this data, discarding it\n");  
      continue;
    }    

    ret=usb_bulk_read(WG_fs_usb, endpoint_in, (char*)&receive_buf[0], 31, 1); /* wait up to 1ms for the data */
    printf("and then %d more bytes\n",ret);
    if(ret != 31)
      /* Failed to read data, we're finished for this tick */
      return; 
#endif
    ...
    if(receive_buf[0] != 'D') {
      printf("I dont understand this data, discarding it\n");
      for(i=0;;i++) {
	ret=usb_bulk_read(WG_fs_usb, endpoint_in, (char*)receive_buf, 1, 1); /* wait up to 10ms for the data */
	if(ret != 1) break;
      }
    ...
    sensor = receive_buf[1] - '0';
    /*printf("Got sensor %d ('%c')\n",sensor,receive_buf[1]);*/
	...
	printf("\n");
      ...
      intA = receive_buf[3] * 256 + receive_buf[2];
      wtrackerSensors[sensor].x = intA * 0.0001;
      intA = receive_buf[5] * 256 + receive_buf[4];
      wtrackerSensors[sensor].y = intA * 0.0001;
      intA = receive_buf[7];
      intA = intA * 256 + receive_buf[6];
      wtrackerSensors[sensor].z = intA * 0.0001;

      intA = receive_buf[9];
      intA = intA * 256 + receive_buf[8];
      wtrackerSensors[sensor].a = intA * 0.01;

      intA = receive_buf[11];
      intA = intA * 256 + receive_buf[10];
      wtrackerSensors[sensor].e = intA * 0.01;

      intA = receive_buf[13];
      intA = intA * 256 + receive_buf[12];
      wtrackerSensors[sensor].r = intA * 0.01;
	
      intA = receive_buf[15];
      intA = intA * 256 + receive_buf[14];
      wtrackerSensors[sensor].qw = intA * 0.01;
      
      intA = receive_buf[17];
      intA = intA * 256 + receive_buf[16];
      wtrackerSensors[sensor].qx = intA * 0.01;
      
      intA = receive_buf[19];
      intA = intA * 256 + receive_buf[18];
      wtrackerSensors[sensor].qy = intA * 0.01;
      
      intA = receive_buf[21];
      intA = intA * 256 + receive_buf[20];
      wtrackerSensors[sensor].qz = intA * 0.01;

      intA = receive_buf[23];
      intA = intA * 256 + receive_buf[22];
      wtrackerSensors[sensor].button = intA;      

      /*printf("sensors[%d].xyz = %3.3f %3.3f %3.3f\n",sensor,wtrackerSensors[sensor].x,wtrackerSensors[sensor].y,wtrackerSensors[sensor].z);*/

    } else {
      printf("Received invalid sensor: %d\n",sensor);
      /* this should essential flush the buffer? */
      /*usb_bulk_read(WG_fs_usb, 32, (char*)receive_buf, 32, 10);*/
    }
  }
}

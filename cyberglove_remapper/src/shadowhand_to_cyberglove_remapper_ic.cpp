...
//own .h
#include "shadowhand_to_cyberglove_remapper_ic.h"
...
const int ShadowhandToCybergloveRemapperIc::number_hand_joints_ = 20;

ShadowhandToCybergloveRemapperIc::ShadowhandToCybergloveRemapperIc() :
  n_tilde_("~")
{
    proj_matrix_.setIdentity(number_hand_joints_,number_hand_joints_);

    joints_names_.resize(number_hand_joints_);
    ShadowhandToCybergloveRemapperIc::initNames();
    ...
    cyberglove_jointstates_sub_ = node_.subscribe(full_topic, 10, &ShadowhandToCybergloveRemapperIc::jointStatesCallback, this);

    n_tilde_.searchParam("remapper_prefix", searched_param);
    n_tilde_.param(searched_param, prefix, std::string());
    full_topic = prefix + "project_eigenspace";
    project_eigenspace_service_ = node_.advertiseService(full_topic,&ShadowhandToCybergloveRemapperIc::formProjMatrix,this);
   
    
 XmlRpc::XmlRpcValue output_topics;
  if (n_tilde_.searchParam("output_topics", searched_param))
    {
      n_tilde_.getParam(searched_param,output_topics);
	...
	{    
	  ROS_ASSERT(output_topics[i].getType() == XmlRpc::XmlRpcValue::TypeString);
          shadowhand_pubs_.push_back(node_.advertise<std_msgs::Float64>(output_topics[i],1));
	}
    }
  else
    {
      ROS_ERROR("No output topics specified - cannot start the Cyberglove Remapper.");
      ROS_BREAK();
    }
...
void ShadowhandToCybergloveRemapperIc::initNames()
{
...
double ShadowhandToCybergloveRemapperIc::linRegression(int ordinal, double sv1, double sv2)
{
...
double ShadowhandToCybergloveRemapperIc::checkJointLimits(double angle,double lower,double upper)
{
  double new_angle;
...
double ShadowhandToCybergloveRemapperIc::linWristMapping(std::string const & joint,double const sv)
{
  double angle=0;
  if(joint=="G_WristPitch")
    ...
    ROS_ERROR("Invalid wrist joint specified in ShadowhandToCybergloveRemapperIc::linWristMapping(std::string const & joint,double const sv)");

  return angle;
}

void ShadowhandToCybergloveRemapperIc::projectOnEspace(Eigen::VectorXd & hand_joints)
{
  ...
  Eigen::VectorXd ShadowhandToCybergloveRemapperIc::getRemappedVector(std::vector<double> const & glove_values)
{
...
void ShadowhandToCybergloveRemapperIc::jointStatesCallback( const sensor_msgs::JointStateConstPtr& msg )
{
    //Do conversion
    ...
    for(unsigned int i=0; i<shadowhand_pubs_.size();i++)
      {
	joint_angle.data=vect(i)/180*PI;
        shadowhand_pubs_[i].publish(joint_angle);
      }

}

  bool ShadowhandToCybergloveRemapperIc::formProjMatrix(cyberglove_remapper::project_eigenspace::Request  &req, cyberglove_remapper::project_eigenspace::Response &res)
 {
  data_mutex_.lock();
